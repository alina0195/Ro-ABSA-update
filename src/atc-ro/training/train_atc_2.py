...
class config:
  ...
  MODEL_SAVE_PATH = ROOT + os.sep +'models/new_models/atc_flanxxl_v1.pt' 
  MODEL_PRETRAINED_ATE ='google/flan-t5-xl' 
  PRE_TRAINED_TOKENIZER_NAME ='google/flan-t5-xxl'
  WANDB_INIT_NAME = "flan-t5-xxl-v1"
  
  MAX_SOURCE_LEN = 512
  MAX_TARGET_LEN = 30
  # PROMPT = 'Extract categories of aspect terms: '
  BATCH_SIZE = 2
  EPOCHS = 5
...
def eval(model, tokenizer, dataloader, epoch, accelerate):
    ...
    for step, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            targets_cleaned = [clean_predictions(t) for t in target]
            
            preds_cleaned_combined = [' or '.join(e) for e in preds_cleaned]
            targets_cleaned_combined = [' or '.join(t) for t in targets_cleaned]

            recalls_current = [recall(preds_cleaned_combined[idx],targets_cleaned_combined[idx]) for idx in range(0,len(target))]
            ...
            if step % 20 == 0 and not step == 0:
              for text,tar,pred in zip(batch['text_cleaned'], targets_cleaned, preds_cleaned):
                  print('Review:', text)
                  print('Target:',tar,'\nPrediction:',pred)
            else:
              print('Targets:',targets_cleaned,'\nPredictions:',preds_cleaned)
            print('\nF1:',f1_val)
...
def train_loop(lr_idx, model, tokenizer, train_dataloader, val_dataloader, optimizer, accelerate, gen_config):
  ...
  for epoch in range(config.EPOCHS):
    ...
    val_loss, val_f1, val_recall, val_other_scores  = eval(model, tokenizer, val_dataloader, epoch, accelerate)
    wandb.log({"Train Loss":train_loss, "Val Loss": val_loss,
               "Val F1": val_f1, "Val Recall":val_recall,
               "Val rouge1":val_other_scores['rouge1'],"Val rouge2":val_other_scores['rouge2'],
               "Val rougeL":val_other_scores['rougeL'],"Val rougeLsum":val_other_scores['rougeLsum'],
               "Val meteor":val_other_scores['meteor'],
               "Val exact match": val_other_scores['exact_match'], "Scheduler":current_lr})
  ...
  test_loss, test_f1, test_recall, test_other_scores  = eval(best_model,tokenizer, test_dataloader, 0, False)
...
train_dataloader, val_dataloader, test_dataloader = get_dataloader(df_train, df_test, df_val, 'text_cleaned', 'all_categories')
